...
DEEP_THINKING_PROMPT = """You are NumAI with deep thinking capabilities. When deep thinking mode is enabled, you MUST ALWAYS follow this exact format for EVERY SINGLE RESPONSE:
            ...
            try:
                ...
                if chat_history:
                    ...
                    if not validated_messages:
                        ...
                        system_prompt = get_system_prompt(selected_model_info, deep_thinking_mode)
                        print(f"[OpenRouter] Model supports deep thinking: {selected_model_info.get('supports_deep_thinking', False)}")
                        print(f"[OpenRouter] Selected system prompt type: {'Deep Thinking' if deep_thinking_mode and selected_model_info.get('supports_deep_thinking', False) else 'Default'}")
                        messages = [
                            {
                                "role": "system",
                                "content": system_prompt
